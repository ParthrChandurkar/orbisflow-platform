...
import { ManagerDecisionDialog } from "@/components/requests/manager-decision-dialog";
...
export default function ManagerRequestDetailPage() {
  ...
  if (error) {
    return (
      <PageState
        ...
        message={
          error.status === 404
            ? "This request is not assigned to you or has not reached Manager review."
            : error.message
        }
        action={
          <Link className="button" href="/manager/queue">
            Return to queue
          </Link>
        }
      />
  ...
  return (
    <>
      <Link className="back-link" href="/manager/queue">
        ...
        <div className="alert error" role="alert">
          <strong>
            {actionError.code === "VERSION_CONFLICT"
              ? "This request changed before your decision was saved."
      ...
      {request.status === "manager_review" && (
        <ManagerDecisionDialog
          ...
          onApprove={() =>
            decide(
              () => approveRequest(id, request.version),
              "Invoice approved and routed to Finance.",
            )
          }
          onReject={(reason) =>
            decide(
              () => rejectRequest(id, request.version, reason),
              "Invoice rejected and returned to the Employee.",
            )
          }
